...
def createnode(node, content,dep,pos,lines):
        ...
        if isinstance(c, SpanNode):
           ...
           # print c[1]
            ...
            if mys in pos.keys():
                ...
                node.pos = pos[mys].strip()

            else:
                mys=mys.replace("<P>","")
              #  print mys
            if mys in lines.keys():
                mLines = []
             #   mLines =lines[mys].split("\t")
             #   for de in mLines:
                mLines.append(lines[mys].strip())
                node.line =mLines
        else:
...
def buildtree(text, fname):
 ...
 #   print fname
  #  st= fname
    if fname.endswith(".out.dis"):
        s= fname.split(".out.dis")
    ...
    while queue:
        ...
        if token == ')':
            ...
            if label == 'Root':
                ...
                node = createnode(node, content,dep,pos,lines)
                stack.append(node)
            elif label == 'Nucleus':
                node = SpanNode(prop=label)
                node = createnode(node, content,dep,pos,lines)
                ...
                stack.append(('orig',createorig(content)))
            else:
...
def backprop(tree,fname):
    ...
    for node in treenodes:
        if (node.lnode is not None) and (node.rnode is not None):
            ...
            node.line = __getlineinfo(node.lnode, node.rnode)
...
def __getlineinfo(lnode, rnode):
        if lnode.line ==None:
            return rnode.line
        if rnode.line ==None:
            return lnode.line
        newdeps =[]
        for lines in lnode.line:
                newdeps.append(lines)

        for lines in rnode.line:
                newdeps.append(lines)
